...
function buildShareText(
  ...
  const efficiency =
    hops === optimal ? '— optimal line 🎯' : `(optimal: ${optLabel})`
...
export function ResultScreen({
  ...
  // Build the optimal path for comparison display
  const optimalArtistPath = findShortestPath(graph, challenge.startId, challenge.destinationId) ?? []

  function handleShare() {
    const text = buildShareText(challenge, path, artists)
  ...
  return (
    <div className="flex-1 px-4 pb-8 pt-2 animate-rise">
      ...
      {/* Optimal path (only if player didn't match it) */}
      ...
      <div className="flex flex-col gap-3 mt-6">
        ...
        <button
          onClick={onPractice}
          className="w-full rounded-2xl border border-white/8 bg-white/4 py-3 text-haze/50 text-sm active:scale-98 transition-transform"
        >
          Practice another
        </button>
